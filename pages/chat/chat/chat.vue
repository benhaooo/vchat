<template>
	<view>
		...
		:style="'bottom:'+KeyboardHeight+'px;'">
			<view class="flex align-center p-1 pb-3" style="background-color: #F7F7F6;">
				...
				<view class="flex-1 mx-2">
					<view v-if="mode === 'audio'" class="rounded flex align-center justify-center" style="width: 100%;" :class="isRecording?'bg-hover-light':'bg-white'" @touchstart="voiceTouchStart" @touchend="voiceTouchEnd" @touchcancel="voiceTouchCancel" @touchmove="voiceTouchMove">
						<text class="font">{{isRecording ? '松开 结束':'按住 说话'}}</text>
					</view>
					
					<textarea v-else fixed auto-height class="bg-white rounded p-2 font-sm" style="width: 100%;" :adjust-position="false" v-model="text" @focus="focus" @blur="blur"/>
				</view>
				<!-- 表情 -->
			...
			<view v-if="mode === 'emoticon' || mode === 'action'" style="height: 580rpx;" class="border-top border-light-secondary bg-light">
				<swiper :indicator-dots="emoticonOrActionList.length > 1" style="height: 510rpx;">
					<swiper-item class="flex flex-wrap justify-between" 
					v-for="(item,index) in emoticonOrActionList"
					:key="index">
						<view class="flex flex-column align-center justify-center" style="height: 255rpx;width: 200rpx;" v-for="(item2,index2) in item" :key="index2" @click="actionEvent(item2)">
