...
<!-- 		<free-nav-bar bgColor="bg-white">
		...
		<free-list-item :cover="user.avatar ? user.avatar : '/static/images/userpic.jpg'" 
		coverSize="120" :title="nickname" showRight @click="open('my/userinfo/userinfo')">
			<view class="flex flex-column m-5">
				<text class="text-dark font-lg font-weight-bold">{{nickname}}</text>
				<text class="text-light-muted font mt-2">微信号：{{user.username}}</text>
			</view>
			<view slot="right">
